...
class Mailbox {
	...
	/**
         ...
         * @throws attachmentDirException
         */
	public function __construct($imapPath, $login, $password, $attachmentsDir = null, $serverEncoding = 'UTF-8') {
		if(!extension_loaded('imap')){
			throw new ExtensionException('imap');
		}
		...
		if($attachmentsDir) {
			if(!is_dir($attachmentsDir)) {
				throw new AttachmentDirException('Directory "' . $attachmentsDir . '" not found');
			}
	...
	public function switchMailbox($imapPath = '') {
		...
		if(!$imapStream) {
			throw new MailBoxException("Couldn't switch  mailbox: " . imap_last_error());
		}
	}
	...
	protected function initImapStream() {
		...
		if(!$imapStream) {
			...
			imap_errors();
			throw new ConnectException('Connection error: ' . $lastError);
		}
		return $imapStream;
	}
	...
	public function getMail($mailId, $markAsSeen = true) {
		...
		$mail->headers = $head;
		$mail->id = $mailId;
		$mail->time = isset($head->date) ? strtotime(preg_replace('/\(.*?\)/', '', $head->date)) : Date::time();
		$mail->date = Date::format('Y/m/d H:i:s', $mail->time);
